...
happy %>% 
  pivot_wider(names_from = factors,
              values_from = value) %>% 
  select(-c(country,year,continent)) %>% 
  cor() %>%
  corrplot(method = 'square', type = "upper", title = "2011-2018") 
...
happy %>% 
  ...
  select(-c(country,year,continent)) %>% 
  cor() %>%
  corrplot(method = 'square', type = "upper", title = "2018") 
...
happy %>% 
  ...
  select(-c(country,year,continent)) %>% 
  cor() %>%
  corrplot(method = 'number', type = "upper", title = "2018") 
...
happy %>% 
  ...
  select(-c(country,year,continent)) %>% 
  cor() %>%
  corrplot(method = 'square', type = "upper", title =  "Australia: 2011-2018") 
...
happy %>% 
  ...
  select(-c(country,year,continent)) %>% 
  cor() %>%
  corrplot(method = 'square', type = "upper", title = "Europe, North America, South America: 2011-2018") 
...
happy %>% 
  ...
  select(-c(country,year,continent)) %>% 
  cor() %>%
  corrplot(method = 'square', type = "upper", title = "Africa, Asia: 2011-2018") 
```
